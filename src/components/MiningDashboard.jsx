...
import { FaChartBar, FaFire, FaChartLine, FaInbox, FaCalendarDay, FaCalendarAlt, FaCalendar, FaChartPie, FaArchive } from "react-icons/fa";
...
const fmtT = (n) => (n / 1000).toFixed(2) + ' t';
...
export default function MiningDashboard({ mineId: lockedMineId = '' }) {
  ...
  if (!rows || rows.length === 0) return (
    <EmptyState icon={<FaInbox />} text={`No emission records found for mine "${lockedMineId}". Submit your first daily log to see the dashboard.`} />
  );
  return (
    <div className="mdb fade-in">
      ...
      <div className="mdb__kpi-grid">
        <KpiCard
          icon={<FaFire />} label="Total CO₂e (All Time)"
          value={fmt(totalCO2)} unit="kg"
          sub={`≈ ${fmtT(totalCO2)}`}
          color="#ef4444"
        />
        <KpiCard
          icon={<FaCalendarDay />} label="Today / Latest Day"
      ...
      <div className="mdb__two-col" style={{ marginBottom: '1.5rem' }}>
        ...
        <ChartPanel title={<><FaChartPie /> All-Time Source Breakdown</>} subtitle="Share of total CO₂e by emission source">
        ...
        <ChartPanel title={<><FaChartBar /> Source Totals (All Time)</>} subtitle="kg CO₂e per emission category">
      ...
      <div className="mdb__tabs">
        <SectionTab id="daily" label="Daily" icon={<FaCalendarDay />} active={view === 'daily'} onClick={setView} />
        <SectionTab id="monthly" label="Monthly" icon={<FaCalendarAlt />} active={view === 'monthly'} onClick={setView} />
        <SectionTab id="yearly" label="Yearly" icon={<FaCalendar />} active={view === 'yearly'} onClick={setView} />
      </div>

      {viewData.length === 0 ? (
        <EmptyState icon={<FaInbox />} text="No data available for this view." />
      ) : (
        <>

          <ChartPanel
            title={<><FaChartLine /> Total CO₂e — {view.charAt(0).toUpperCase() + view.slice(1)} Trend</>}
          ...
          <ChartPanel
            title={<><FaChartBar /> Emissions by Source — {view.charAt(0).toUpperCase() + view.slice(1)} Breakdown</>}
          ...
          <ChartPanel
            title={<><FaChartLine /> Source-wise Trend Lines — {view.charAt(0).toUpperCase() + view.slice(1)}</>}
            ...
            <div className="mdb__chart-header">
              <div>
                <div className="mdb__chart-title">
                  <FaArchive /> {view.charAt(0).toUpperCase() + view.slice(1)} Summary Table
                </div>
